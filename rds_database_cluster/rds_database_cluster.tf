...
resource "aws_rds_cluster" "rds_db_cluster" {
  ...
  cluster_identifier = lookup(
    var.optional_parameters,
    "cluster_identifier",
    "${var.custom_identifier}-cluster",
  )
  ...
  iam_database_authentication_enabled = lookup(
    var.optional_parameters,
